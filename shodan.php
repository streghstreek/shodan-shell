...
$key = "U5Ld0rbM56LyEll5NdB80EHrXJWmCKzG";
...
for ($page = 1; $page <= 50; $page++) {
    ...
    echo "\n \033[91m Making Shodan API request for Page = $page\n";
    $data = json_decode(file_get_contents($api . $page), 1);
    echo "Got the response from Shodan\n\n";

    if (!isset($data["matches"])) {
        echo $api . $page . "\n\n";
        echo "\033[91m<=== Problem with Shodan API. Open above link in browser to see the error ===>\n\n";
        exit;
    }

    echo str_repeat("\033[92m#", 60) . "\n";
    echo "Starting to analyse the results....\n";
    echo str_repeat("#", 60) . "\n\n";
    $results = $data["\033[93mmatches"];
